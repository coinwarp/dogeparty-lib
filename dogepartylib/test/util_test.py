...
import server
from dogepartylib.lib import (config, api, util, exceptions, blocks, check, backend, database, transaction, script)
from dogepartylib.lib.messages import (send, order, btcpay, issuance, broadcast, bet, dividend, burn, cancel, rps, rpsresolve)
...
def run_scenario(scenario, rawtransactions_db):
    ...
    for tx in scenario:
        if tx[0] != 'create_next_block':
            module = sys.modules['dogepartylib.lib.messages.{}'.format(tx[0])]
...
def check_outputs(tx_name, method, inputs, outputs, error, records, server_db):
    ...
    try:
        tested_module = sys.modules['dogepartylib.lib.{}'.format(tx_name)]
    except KeyError:    # TODO: hack
        tested_module = sys.modules['dogepartylib.lib.messages.{}'.format(tx_name)]
    ...
    if error is not None:
        ...
        if pytest.config.option.gentxhex and method == 'compose':
            ...
            if tx_name == 'order' and inputs[1]=='DOGE':
                print('give btc')
...
def reparse(testnet=True):
    ...
    data_dir = appdirs.user_data_dir(appauthor=config.XDP_NAME, appname=config.APP_NAME, roaming=True)
