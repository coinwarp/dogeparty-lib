...
from dogepartylib.lib import config, util, backend, transaction
...
def init_mock_functions(monkeypatch, rawtransactions_db):
    ...
    monkeypatch.setattr('dogepartylib.lib.backend.get_unspent_txouts', get_unspent_txouts)
    monkeypatch.setattr('dogepartylib.lib.log.isodt', isodt)
    monkeypatch.setattr('dogepartylib.lib.log.curr_time', curr_time)
    monkeypatch.setattr('dogepartylib.lib.util.date_passed', date_passed)
    monkeypatch.setattr('dogepartylib.lib.api.init_api_access_log', init_api_access_log)
    if hasattr(config, 'PREFIX'):
        monkeypatch.setattr('dogepartylib.lib.config.PREFIX', b'TESTXXXX')
    monkeypatch.setattr('dogepartylib.lib.backend.getrawtransaction', get_cached_raw_transaction)
    monkeypatch.setattr('dogepartylib.lib.backend.pubkeyhash_to_pubkey', pubkeyhash_to_pubkey)
    monkeypatch.setattr('dogepartylib.lib.backend.multisig_pubkeyhashes_to_pubkeys', multisig_pubkeyhashes_to_pubkeys)
